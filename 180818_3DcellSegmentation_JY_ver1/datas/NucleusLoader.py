...
class NSDataset(data.Dataset):
    ...
    def _3D_image(self, idx):
        ...
        input_np=input_np[:208,:208,5:181]
        target_np=data_pack['target']
        target_np=target_np[:208,:208,5:181]
        weight_np=data_pack['weight']
        weight_np=weight_np[:208,:208,5:181]



        #if idx >= self.origin_image_len:
        for t in self.transform:
            input_np, target_np, weight_np = t(input_np, target_np, weight_np)

        input_  = self._np2tensor(input_np[:,:,:].copy())
        target_ = self._np2tensor(target_np[:,:,:].copy())
        weight_ = self._np2tensor(weight_np[:,:,:].copy())
        return input_[None], target_[None], weight_[None], os.path.basename(img_path)
