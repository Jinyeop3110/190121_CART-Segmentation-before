...
#        gc_fm1 = self.ConBR1(torch.cat((fm1, gc_fm2), 1))
        ...
        fs3 = self.ConBR3(F.interpolate(fs4,size=gc_fm3.size()[2:],mode='trilinear', align_corners=True) + gc_fm3)
        fs2 = self.ConBR2(F.interpolate(fs3,size=gc_fm2.size()[2:],mode='trilinear', align_corners=True) + gc_fm2)
        fs1 = self.ConBR1(F.interpolate(fs2,size=gc_fm1.size()[2:],mode='trilinear', align_corners=True) + gc_fm1)
