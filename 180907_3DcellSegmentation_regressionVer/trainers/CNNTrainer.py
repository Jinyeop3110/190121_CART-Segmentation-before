...
class CNNTrainer(BaseTrainer):
    ...
    def train(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.start_epoch, self.epoch):
            for i, (input_, target_, weight_,_) in enumerate(train_loader):
                self.G.train()
                input_, target_, weight_= input_.to(self.torch_device), target_.to(self.torch_device), weight_.to(self.torch_device)
                output_ = self.G(input_)
                recon_loss = self.recon_loss(output_, target_, weight_)
    ...
    def valid(self, epoch, val_loader):
        ...
        with torch.no_grad():
            losssum=0
            count=0;
            for i, (input_, target_, weight_, _) in enumerate(val_loader):
                if (i > 30):
                    break
                input_, target_, weight_= input_.to(self.torch_device), target_.to(self.torch_device), weight_.to(self.torch_device)
                _, output_, target_ = self._test_foward(input_, target_)
                loss=self.recon_loss(output_,target_, weight_)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            for i, (input_, target_, weight_, fname) in enumerate(test_loader):
